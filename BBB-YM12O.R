...
Middle1 <-  CreateSeuratObject(counts = Read10X(data.dir = "/md01/nieyg/project/BBB/data/Middle_1/Middle_count/outs/filtered_feature_bc_matrix"), 
	project="Middle1", assay = "RNA")
Middle2 <-  CreateSeuratObject(counts = Read10X(data.dir = "/md01/nieyg/project/BBB/data/Middle_2/Mid1-count/outs/filtered_feature_bc_matrix"), 
	project="Middle2", assay = "RNA")

#####添加sample信息####
objList <- list(Young,Old,Middle1,Middle2)
...
Middle1[["percent.mt"]] <- PercentageFeatureSet(Middle1, pattern = "^mt-")
Middle2[["percent.mt"]] <- PercentageFeatureSet(Middle2, pattern = "^mt-")

# Visualize QC metrics as a violin plot
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/QC/YM12O_qc_plot_nopoint.pdf")
VlnPlot(Young, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size = 0,)
VlnPlot(Middle1, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size = 0,)
VlnPlot(Middle2, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size = 0,)
VlnPlot(Old, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size = 0,)

# FeatureScatter is typically used to visualize feature-feature relationships, but can be used
# for anything calculated by the object, i.e. columns in object metadata, PC scores etc.
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/QC/YM12O_qc_plot_2.pdf")
...
Middle1 = subset(Middle1,nFeature_RNA >350 & nFeature_RNA < 4000 & percent.mt < 15 )
Middle2 = subset(Middle2,nFeature_RNA >350 & nFeature_RNA < 4000 & percent.mt < 15 )
Old = subset(Old,nFeature_RNA >350 & nFeature_RNA < 4000 & percent.mt < 15 )

# Simply merge Seurat objects
merged_obj <- merge(x=Young,y=c(Middle1,Middle2,Old),add.cell.ids = c("Young","Middle1","Middle2","Old"),project = "BBB_YM12O")
...
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/Young_normalization_plot.pdf")
...
plot2
dev.off();
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/Middle1_normalization_plot.pdf")
...
plot2
dev.off();
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/Middle2_normalization_plot.pdf")
...
plot2
dev.off();
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/Old_normalization_plot.pdf")
BBB_EC.list[[4]] <- NormalizeData(BBB_EC.list[[4]],verbose = FALSE)
BBB_EC.list[[4]] <- FindVariableFeatures(BBB_EC.list[[4]],selection.method = "vst", nfeatures = 2000, verbose = FALSE)
top10 <- head(VariableFeatures(BBB_EC.list[[4]]),10)
plot1 <- VariableFeaturePlot(BBB_EC.list[[4]])
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
plot1
plot2
dev.off();
...
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/Umap-sample.pdf.pdf")
...
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/seclect_pca.pdf")
JackStrawPlot(BBB_EC.integrated, dims = 1:50)
# ‘Elbow plot’
ElbowPlot(BBB_EC.integrated,ndims=50)
dev.off()

saveRDS(BBB_EC.integrated,"BBB_YM12O.integrated.rds")
...
library(ggplot2)
library(dplyr)
BBB_EC.integrated<-readRDS("BBB_YM12O.integrated.rds")

pdf("/md01/nieyg/project/BBB/YM12O_results/plot/cell_cluster.pdf")
#※细胞聚类
...
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/Cluster_Tree.pdf")
...
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/BBB_cluster_annotation-all.pdf",width=16,height=8)
...
new.cluster.ids <- c("Capillary","C_V","Capillary","C_A","Capillary","Arterial","Interferon","Venous","Capillary","Type2_neuron","Capillary",
	"Choroid_plexus","Pericyte","Hb_VC","Microglia","Oligodendrocyte","Lymphatics")
names(new.cluster.ids) <- levels(BBB_EC.integrated)
BBB_EC.integrated <- RenameIdents(BBB_EC.integrated, new.cluster.ids)
pdf("/md01/nieyg/project/BBB/YM12O_results/plot/Allcell_add_celltype_umap.pdf")
