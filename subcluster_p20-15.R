...
# EC_cells <- subset(BBB_EC.integrated,idents=c(0:12))
# EC_cells <- DietSeurat(
#   object=EC_cells,
#   counts=TRUE,
#   data=TRUE,
...
# pdf("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/integrated_EC_cells_pc.pdf")
# JackStrawPlot(EC_cells.integrated, dims = 1:50)
# ElbowPlot(EC_cells.integrated,ndims=50)
# dev.off()
EC_cells.integrated <- readRDS("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/EC_cell.integrated350-4000_15_25_0.5_abc.rds")
pdf("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/EC_cluster350-4000_15_20_0.5.pdf")
#※细胞聚类
EC_cells.integrated <- FindNeighbors(object = EC_cells.integrated, dims = 1:20)
EC_cells.integrated <- FindClusters(object = EC_cells.integrated, resolution = 0.5)
...
EC_cells.integrated <- RunTSNE(object = EC_cells.integrated, dims = 1:20)
DimPlot(object = EC_cells.integrated, reduction = "tsne",pt.size=0.1,label = T)
#group.by="seurat_clusters" group.by="celltype"

dev.off()
saveRDS(EC_cells.integrated, file = "/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/EC_cell.integrated350-4000_15_20_0.5_abc.rds")
...
table(EC_cells.integrated@meta.data$orig.ident,EC_cells.integrated$seurat_clusters) #查看3个样品不同cluster有多少个细胞


#cluster annotation
#基因占比图
pdf("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/cluster_annotation1.pdf")
...
pdf("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/cluster_annotation2.pdf")
...
library("ggsci")
pdf("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/barplot_ECcell_cluster.pdf")
...
#23个基因在不同年龄的表达量热图
pdf("/public/home/yangjw28/projects/BBB/BBB_sc/YMO/23_heatmap_cluster.pdf")
for (i in seq(0,17)){
...
pdf("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/EC_cluster350-4000_15_15_0.5.pdf")
EC_cells.integrated <- FindNeighbors(object = EC_cells.integrated, dims = 1:15)
EC_cells.integrated <- FindClusters(object = EC_cells.integrated, resolution = 0.5)
...
#执行非线性降维
EC_cells.integrated <- RunUMAP(object = EC_cells.integrated, dims = 1:15)
DimPlot(object = EC_cells.integrated, group.by = "orig.ident",label = T)
DimPlot(object = EC_cells.integrated, split.by = "orig.ident",label = T)
table(EC_cells.integrated@meta.data$orig.ident,EC_cells.integrated$seurat_clusters)
#不同年龄细胞在不同cluster的分布
EC_cells.integrated <- RunUMAP(object = EC_cells.integrated, dims = 1:15)
DimPlot(object = EC_cells.integrated, reduction = "umap",label = T)
EC_cells.integrated <- RunTSNE(object = EC_cells.integrated, dims = 1:15)
DimPlot(object = EC_cells.integrated, reduction = "tsne",pt.size=0.1,label = T)
#group.by="seurat_clusters" group.by="celltype"

dev.off()
saveRDS(EC_cells.integrated, file = "/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/EC_cell.integrated350-4000_15_15_0.5_abc.rds")
#EC_cell.integrated <- readRDS("EC_cell.integrated350-4000_15_25_0.5_abc.rds")
#分clustrer正常？
table(EC_cells.integrated@active.ident) # 查看每一cluster有多少个细胞
table(EC_cells.integrated@meta.data$orig.ident,EC_cells.integrated$seurat_clusters) #查看3个样品不同cluster有多少个细胞
...
#cluster annotation
#基因占比图
pdf("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/cluster_annotation1_p15.pdf")
features <- c( "Fbln5", "Cytl1","Mgp","S100a6", "Azin1","Pi16","Bmx", "Vegfc","Fbln2","Gkn3","Hey1","Edn3","Tgfb2", "Glul","Slc26a10","Lypd1","Ddc","Mfsd2a", "Cxcl12","Spock2","Rgcc","Tfrc", "Car4","Itm2a","Chn2","Lcn2","Slc38a5","Nr2f2", "Sox12","Tbx1", "Tmbs10","Icam1","Vcam1", "Vwf", "P2ry1")
DotPlot(EC_cells.integrated, features = features,dot.scale = 3) + RotatedAxis()+ theme(axis.text.x=element_text(size=8))
dev.off()

pdf("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/cluster_annotation2_p15.pdf")
features <- c( "Plvap", "Plpp3","Esm1","Pdgfrb", "Cspg4","Kcnj8","Isg15", "Ifit1","Ifit3","Ifit3b","Prox1", "Pdpn","Acta2", "Pdlim3","Myh11","Aif1","Csf1r", "Cd68","Dcn", "Pdgfra","Mbp","Opalin", "Mobp","Syt1","Syp", "Eno2")
DotPlot(EC_cells.integrated, features = features,dot.scale = 3) + RotatedAxis() + theme(axis.text.x=element_text(size=8))
dev.off()
...
#3个年龄样本细胞数目占比的barplot
library("ggsci")
pdf("/public/home/yangjw28/projects/BBB/BBB_sc/EC_cell/barplot_ECcell_cluste_p15r.pdf")
ggplot(EC_cells.integrated@meta.data, aes(x=orig.ident, fill=seurat_clusters)) + geom_bar(position="fill") + scale_fill_igv()
#+coord_flip()
#ggplot(BBB_EC.integrated@meta.data, aes(x=orig.ident, fill=seurat_clusters)) + geom_bar()
dev.off()
...
#23个基因在不同年龄的表达量热图
pdf("/public/home/yangjw28/projects/BBB/BBB_sc/YMO/23_heatmap_cluster_p15.pdf")
for (i in seq(0,17)){
  subBBB_EC.integrated<-subset(x = EC_cells.integrated,idents=i)
  features = c('Bsg','Abcg2','Abcb1a',"Slc16a4",'Slc30a1','Slc16a1','Slco1c1','Slc2a1',"Gpd2",'Nt5c2','Ddc','Eogt','Isyna1','Ocln','Cgnl1','Sorbs2','Dnm3','Palm','Tfrc','Igf1r','Tsc22d1','Esyt2','Palmd')
  p <-DoHeatmap(subBBB_EC.integrated, features = features, group.by = "orig.ident") 
