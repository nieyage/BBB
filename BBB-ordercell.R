...
EC_five<-subset(EC_cells.integrated, idents = c( "Arterial","C_A","Capillary_1","Capillary_2","Capillary_3","Venous","C_V_1","C_V_2"))

"Capillary_4",

EC_five<-subset(EC_cells.integrated, idents = c( ,"Capillary_1","Capillary_2","Capillary_3"))


#####merge Capillary
#EC_five@meta.data$subtype<-Idents(EC_five)
#new.cluster.ids <- c("Arterial","C_A","Capillary","Capillary","Capillary","Capillary")
#names(new.cluster.ids) <- levels(EC_five)
#EC_five <- RenameIdents(EC_five, new.cluster.ids)
#EC_five$celltype<-EC_five@active.ident
#分3个cluster的结果Capillary在两端，不行
############pesudotime rm  
...
monocle_cds <- setOrderingFilter(monocle_cds, 
...
#Trajectory step 2: reduce data dimensionality  max_components = 2,
...
########提取pesudotime
monocle_cds@phenoData @data$Pseudotime


pdf("monocle-BBB-V8.pdf")
plot_cell_trajectory(monocle_cds, color_by = "seurat_clusters",cell_size=0.1)
plot_cell_trajectory(monocle_cds, color_by = "celltype",cell_size=0.1)
plot_cell_trajectory(monocle_cds, color_by = "celltype",cell_size=0.1) +
...
dev.off()
 pdf("monocle-BBB-CA-V4.pdf")
plot_cell_trajectory(monocle_cds, color_by = "seurat_clusters",cell_size=0.1)
plot_cell_trajectory(monocle_cds, color_by = "celltype",cell_size=0.1)
plot_cell_trajectory(monocle_cds, color_by = "celltype",cell_size=0.1) +
    facet_wrap(~celltype, nrow =4)
plot_cell_trajectory(monocle_sub, color_by = "State",cell_size=0.1)
plot_cell_trajectory(monocle_sub, color_by = "State",cell_size=0.1) +
    facet_wrap(~State, nrow =4)
plot_cell_trajectory(monocle_cds, color_by = "Pseudotime",cell_size=0.1)
plot_cell_trajectory(monocle_cds, color_by = "seurat_clusters",cell_size=0.1) +
...
subset(featureData(monocle_cds)@data,num_cells_expressed >= 10)

The23genes <- row.names(subset(fData(monocle_cds),
                               gene_short_name %in% c('Bsg','Abcg2','Abcb1a',"Slc16a4",'Slc30a1','Slc16a1',
                                'Slco1c1','Slc2a1',"Gpd2")))
#,"Nt5c2",'Ddc','Eogt','Isyna1','Ocln','Cgnl1','Sorbs2',
#                                'Dnm3','Palm','Tfrc','Igf1r','Tsc22d1','Esyt2',"Palmd"

plot_genes_branched_pseudotime(monocle_cds[The23genes,],
                               branch_point = 1,ncol=2,
                               color_by = "celltype",cell_size=0.10)

cds_subset <- monocle_cds[The23genes,]
plot_genes_in_pseudotime(cds_subset, color_by = "celltype",cell_size=0.10)
